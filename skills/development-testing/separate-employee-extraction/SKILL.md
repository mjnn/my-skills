---
name: separate-employee-extraction
version: 1.0.0
description: |
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:USERPROFILE\.qoder-cn\tools\skill-kit\scripts\check_skill_kit_ready.ps1" -SkillDir "$env:USERPROFILE\.qoder-cn\skills\separate-employee-extraction"
```
...
| 依赖类型 | 工具/环境 | 用途 | 安装方式 |
|---------|----------|------|---------|
| 必需 | Python 3.8+ | 运行辅助脚本（merge_research.py, srt_to_transcript.py, quality_check.py） | [python.org](https://python.org) |
| 必需 | yt-dlp | 从 YouTube 下载视频字幕 | `vendor/python-packages/yt_dlp/` 离线包；用 `scripts/download_subtitles.ps1` |
| 可选 | Bash/Zsh | 执行 download_subtitles.sh | 默认已安装 |
| 可选 | Gemini Video Skill | 分析本地视频文件并提取转录 | 通过 Qoder 插件安装 |
| 可选 | Web Article Reader Skill | 精确读取网页文章全文 | 通过 Qoder 插件安装 |
| 可选 | Agent Reach Skill | 多渠道信息获取（17个平台） | 通过 Qoder 插件安装 |
| 可选 | Huashu Research Skill | 结构化深度调研 | 通过 Qoder 插件安装 |
| 可选 | PDF Skill | 读取 PDF 书籍/论文 | 通过 Qoder 插件安装 |

**注意**：调研脚本依赖 skill-kit 便携 Python；yt-dlp 须在 `vendor/python-packages/` 离线打包。禁止 `pip install`。
